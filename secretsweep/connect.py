"""Native cognis-connect emit for secretsweep — forward findings to any platform.

Maps secretsweep's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    secretsweep ... --format json | secretsweep-emit --to stix
    secretsweep-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('pattern', '') + ' in ' + rec.get('file', '')
        out['severity'] = 'low'
        out['type'] = 'secret'
        out['description'] = f"Found {rec.get('pattern', '')} in {rec.get('file', '')}"
        out['tags'] = [rec.get('provider', ''), rec.get('category', '')]
